...
#   1) Pycharm과 DB를 서로 Connection 맺기
#       - IP: 해당 컴퓨터를 찾아갈 수 있는 주소 (window + R → cmd 누르고 엔터 → ipconfig 입력 후 IPv4주소 읽기)
...
# ** MongoDB 설정 방법
# 1. 직접 설치(Local)
# 2. MongoDB에서 제공하는 웹 클라우드 사용(설치 필요 X)
#    - MongoDB 회원가입 필수
#    - IP와 PORT => URL 제공
...
def conn_mongodb():
    ...
    DB_PW = "1234"  # 상수의 사용 용도: ex) 은행권 => 금리(상수)
    client = MongoClient(f"mongodb+srv://{DB_ID}:{DB_PW}@daumcluster.7cj6hpf.mongodb.net/")
    #DB설정 -> Colllection설정
    db = client["daum"]
